...
HomeGPT is a local-only AI assistant with persistent markdown-based memory and optional autonomous operation via heartbeat.
...
- **config/** - TOML configuration at `~/.homegpt/config.toml`
  - Supports `${ENV_VAR}` expansion in API keys
  - `workspace_path()` returns expanded memory workspace path
  - `migrate.rs` - Auto-migrates from OpenClaw's `~/.openclaw/config.json5` if HomeGPT config doesn't exist
...
Default config path: `~/.homegpt/config.toml` (see `config.example.toml`)

**Auto-creation**: If no config file exists on first run, HomeGPT automatically creates a default config with helpful comments. If an OpenClaw config exists at `~/.openclaw/config.json5`, it will be migrated automatically.
...
- `memory.workspace` - Workspace directory path. Default: `~/.homegpt/workspace`
...
export HOMEGPT_WORKSPACE=~/my-project/ai-workspace
homegpt chat

# Use profile-based workspaces (like OpenClaw's OPENCLAW_PROFILE)
export HOMEGPT_PROFILE=work    # uses ~/.homegpt/workspace-work
export HOMEGPT_PROFILE=home    # uses ~/.homegpt/workspace-home
```
...
1. **Workspace skills**: `~/.homegpt/workspace/skills/` (highest priority)
2. **Managed skills**: `~/.homegpt/skills/` (user-level, shared across workspaces)
...
HomeGPT maintains strong compatibility with OpenClaw workspace files for seamless migration.

### Directory Structure

```
~/.homegpt/                          # State directory (OpenClaw: ~/.openclaw/)
...
## Key Decisions
- Using Rust for HomeGPT implementation
...
HomeGPT uses the same chunking and indexing approach as OpenClaw:
...
cp -r ~/.openclaw/workspace/* ~/.homegpt/workspace/

# Copy session data
cp -r ~/.openclaw/agents ~/.homegpt/agents
...
- No multi-channel routing (HomeGPT is local-only)
- No remote channels (Telegram, Discord, Slack)
- No subagent spawning (single "main" agent)
- No plugin/extension system

**Auto-config migration:**
If `~/.homegpt/config.toml` doesn't exist but `~/.openclaw/config.json5` does:
...
HomeGPT auto-creates `.gitignore` files. Recommended version control:
