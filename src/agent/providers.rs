        ...
        "anthropic" => {
            let anthropic_config = config.providers.anthropic.as_ref().ok_or_else(|| {
                anyhow::anyhow!(
                    ...
                    Set ANTHROPIC_API_KEY env var or add to ~/.homegpt/config.toml:\n\n\
                    [providers.anthropic]\n\
                    api_key = \"sk-ant-...\""
                )
            })?;
        ...
        "openai" => {
            let openai_config = config.providers.openai.as_ref().ok_or_else(|| {
                anyhow::anyhow!(
                    ...
                    Set OPENAI_API_KEY env var or add to ~/.homegpt/config.toml:\n\n\
                    [providers.openai]\n\
                    api_key = \"sk-...\""
                )
            })?;
...
pub struct ClaudeCliProvider {
    ...
    session_key: String,
    /// HomeGPT session ID (for session store tracking)
    homegpt_session_id: String,
...
impl ClaudeCliProvider {
    pub fn new(command: &str, model: &str, workspace: std::path::PathBuf) -> Result<Self> {
        ...
        Ok(Self {
            ...
            workspace,
            session_key,
            homegpt_session_id: uuid::Uuid::new_v4().to_string(),
...
impl LLMProvider for ClaudeCliProvider {
    ...
    async fn chat_stream(
        ...
        let homegpt_session_id = self.homegpt_session_id.clone();
